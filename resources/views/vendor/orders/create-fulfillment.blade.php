...
<div class="mx-auto max-w-2xl pb-[max(4.25rem,env(safe-area-inset-bottom))] max-md:pb-[max(11rem,env(safe-area-inset-bottom))] md:pb-0">
    ...
    <form id="orderWizard_fulfillment_form"
          action="{{ route('vendor.orders.create.fulfillment.store') }}"
          method="POST"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fulfillmentForm = document.getElementById('orderWizard_fulfillment_form');
    ...
    const pickupField = initDateTimeField({
        dateId: 'pickup_date',
        timeSelectId: 'pickup_time_select',
        hiddenId: 'pickup_at',
        prefillTime: @json($pickupParts['time']),
        onValueChange: function (value) {
            if (!fulfillmentForm || typeof Alpine === 'undefined') return;
            const data = Alpine.$data(fulfillmentForm);
            if (data) {
                data.pickupAt = value;
                data.fulfillmentStepError = '';
            }
        },
    });
    window.__orderWizardPickupField = pickupField;

    const deliveryField = initDateTimeField({
        dateId: 'delivery_date',
        timeSelectId: 'delivery_time_select',
        hiddenId: 'delivery_at',
        prefillTime: @json($deliveryParts['time']),
    });

    if (fulfillmentForm) {
        fulfillmentForm.addEventListener('submit', function () {
            pickupField?.sync();
            deliveryField?.sync();
        });
    }
