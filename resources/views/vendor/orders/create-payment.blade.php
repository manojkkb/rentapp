...
<div class="mx-auto max-w-2xl pb-[max(4.25rem,env(safe-area-inset-bottom))] max-md:pb-[max(11rem,env(safe-area-inset-bottom))] md:pb-0" x-data="orderWizardPaymentPage(@js($paymentPreview))">
    @include('vendor.orders.partials.wizard-steps', ['current' => 5, 'compact' => true])
    ...
    <form action="{{ route('vendor.orders.create.complete') }}" method="POST" class="space-y-4 rounded-xl border border-gray-200/90 bg-white p-3 shadow-sm sm:space-y-5 sm:p-4">
        @csrf
