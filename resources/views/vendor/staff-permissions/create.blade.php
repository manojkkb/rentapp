...
@section('title', __('vendor.staff_permissions_add_role'))
@section('page-title', __('vendor.staff_permissions_add_role'))
...
<div class="mx-auto max-w-3xl space-y-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
    <header>
        ...
        <h1 class="text-lg font-bold text-gray-900 sm:text-xl">{{ __('vendor.staff_permissions_add_role') }}</h1>
        <p class="mt-0.5 text-sm text-gray-600">{{ __('vendor.staff_permissions_create_subtitle') }}</p>
    </header>
    ...
    @include('vendor.staff-permissions.partials.role-form')
</div>
@endsection
